...
contract MoeRouterAdapter is ReentrancyGuard {
    ...
    function swapExactTokensForTokens(
        ...
        IERC20(path[0]).forceApprove(address(moe), gotIn);
        amounts = moe.swapExactTokensForTokens(gotIn, amountOutMin, path, to, deadline);
    ...
    function swapExactTokensForETH(
        ...
        uint256 gotIn = IERC20(path[0]).pullMeasured(msg.sender, amountIn);
        IERC20(path[0]).forceApprove(address(moe), gotIn);
        amounts = moe.swapExactTokensForNative(gotIn, amountOutMin, path, to, deadline);
        IERC20(path[0]).forceApprove(address(moe), 0);
    }
