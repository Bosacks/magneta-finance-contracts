...
const DEPLOY_DIR = path.join(__dirname, "..", "..", "deployments-b");
...
async function main() {
  ...
  for (const peerNet of ALL_NETWORKS) {
    ...
    // Register gateway peer — idempotent (skip if already set, resume-safe over
    // ~380 setPeer with flaky RPC). Read peers() with a small retry (new-deploy lag).
    const peerBytes32 = ethers.zeroPadValue(peerGateway, 32);
    let already = false;
    for (let i = 1; i <= 5; i++) {
      try { already = (await gateway.peers(dstEid) as string).toLowerCase() === peerBytes32.toLowerCase(); break; }
      catch { if (i === 5) break; await new Promise((r) => setTimeout(r, 4000)); }
    }
    if (!already) { const tx = await gateway.setPeer(dstEid, peerBytes32, ov); await tx.wait(); }
    console.log(`  ✓ Gateway peer: ${peerNet} (EID ${dstEid})${already ? " already" : " → " + peerGateway}`);
  ...
  // EID ↔ Chain ID mappings — one batch tx (idempotent to re-run).
  const eids: number[] = [];
  ...
  for (const peerNet of ALL_NETWORKS) {
    ...
    eids.push(NETWORK_EID[peerNet]);
    chainIds.push(parseInt(peerDeploy.chainId));
  }
  if (eids.length) {
    await (await gateway.setEidMappingBatch(eids, chainIds, ov)).wait();
    console.log(`  ✓ EID mappings batch: ${eids.length} entries`);
  }
