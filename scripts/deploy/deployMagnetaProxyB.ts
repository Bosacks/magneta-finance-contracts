...
const DEPLOY_DIR = path.join(REPO_ROOT, "deployments-b");
...
async function main() {
  ...
  if (dep.contracts?.MagnetaProxy) {
    console.log(`[${net}] MagnetaProxy already ${dep.contracts.MagnetaProxy} — skip`);
    return;
  }
  ...
  const safe: string | undefined = dep.gnosisSafe ?? dep.safe;
