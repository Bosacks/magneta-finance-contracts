...
async function main() {
  ...
  // Idempotent: skip if already deployed (re-run safe, one-shot script).
  const _dp = path.join(__dirname, "..", "..", "deployments-b", `${network.name}.json`);
  const _ex = fs.existsSync(_dp) ? JSON.parse(fs.readFileSync(_dp, "utf8")) : {};
  if (_ex.contracts?.MagnetaCurveFactory) {
    console.log(`MagnetaCurveFactory already ${_ex.contracts.MagnetaCurveFactory} — skip`);
    return;
  }
