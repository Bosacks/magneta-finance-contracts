...
// TraderJoeAvaxAdapter is a stateless transient-custody UniV2 facade: pull
// tokens/native in -> approve router -> call TraderJoe (WAVAX/addLiquidityAVAX/
// swapExactAVAXForTokens/swapExactTokensForAVAX naming) -> refund dust -> reset
// allowance to zero. This suite backs it with MockUniV2NativeRouter, which
// implements the AVAX-named entrypoints against a shared, configurable mock AMM.
describe("TraderJoeAvaxAdapter", function () {
  let adapter: any;
  let router: any;
  let weth: any;
  let tokenA: any;
  ...
  describe("constructor", function () {
    it("reverts on zero router", async function () {
      const Adapter = await ethers.getContractFactory("TraderJoeAvaxAdapter");
      await expect(Adapter.deploy(ethers.ZeroAddress)).to.be.revertedWith("TJoeAdapter: zero router");
    });

    it("reverts if router.factory() returns zero", async function () {
      await router.setZeroFactory(true);
      const Adapter = await ethers.getContractFactory("TraderJoeAvaxAdapter");
      await expect(Adapter.deploy(await router.getAddress())).to.be.revertedWith("TJoeAdapter: bad router");
    });

    it("reverts if router.WAVAX() returns zero", async function () {
      const Router = await ethers.getContractFactory("MockUniV2NativeRouter");
      const badRouter = await Router.deploy(ethers.ZeroAddress);
      await badRouter.waitForDeployment();
      const Adapter = await ethers.getContractFactory("TraderJoeAvaxAdapter");
      await expect(Adapter.deploy(await badRouter.getAddress())).to.be.revertedWith("TJoeAdapter: bad router");
    });

    it("wires immutables from the router", async function () {
      expect(await adapter.joe()).to.equal(await router.getAddress());
